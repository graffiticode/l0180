...
- **Fill in the blank** — `text-entry [ text "The capital of France is {{capital}}." responses [[id "capital" accept ["Paris"]]] {} ]`. The `{{id}}` marker positions the blank and names it. List every spelling you will take in `accept`; capitals are ignored unless you add `case-sensitive true`.
- **Several blanks** — one marker and one response each. Each is worth a point, so it gives partial credit; wrap it in a conjunctive item for all-or-nothing.
- **A written answer** — `extended-text [ prompt "…" rubric [[score 2 descriptor "…"] [score 0 descriptor "…"]] {} ]`. Nothing scores it; the rubric ships with it and a person marks it. Put it in an additive item so the rest still scores.
